...
bool OpenNiDevice::grab(void)
{
    ...
    if (_color.isValid())
    {
        ...
        const openni::RGB888Pixel* src = reinterpret_cast<const openni::RGB888Pixel*>(_frameColor.getData());
        const int size = _width * _height;
        unsigned char* des = _imgRgb.data;
    ...
    if (_ir.isValid())
    {
        ...
        const uint16_t* src = reinterpret_cast<const uint16_t*>(_frameIr.getData());
        const int size = _width * _height;
        unsigned char* des = _imgIr.data;
